...
require_once('../../helpers/reportFactura.php');

// Se instancia la clase para crear el reporte.
$pdf = new ReportFactura;
...
if ($dataOrders = $Pedidos->readAllOrdersFactura()) {
    if ($dataCliente = $Clientes->readCliente()) {
        // Se inicia el reporte con el encabezado del documento.
        $pdf->startReport(
            'Reporte FeasVerse de tu factura del pedido: ',
            'Factura de tu pedido de zapatos FeasVerse',
            '',
            70,
            10,
            35,
            'idCliente',
            $dataCliente['direccion_cliente'],
            $dataCliente['telefono_cliente'],
            $dataCliente['correo_cliente'],
            $dataCliente['dui_cliente'],
            $dataCliente['apellido_cliente']
        );
        ...
        foreach ($dataOrders as $rowOrders) {
            ...
            if ($Pedidos->setIdPedidoCliente($rowOrders['id_pedido_cliente'])) {
                ...
                if ($dataProductos = $Pedidos->readShoesOfOrders()) {
                    ...
                    $pdf->SetTextColor(0, 0, 0); // Color de texto blanco (RGB)
